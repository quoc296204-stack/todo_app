...
class _TaskDetailPageState extends State<TaskDetailPage> {
  ...
  void initState() {
    super.initState();
    _loadCategories(); // Tải danh sách danh mục ngay khi mở trang
  }

  // Hàm tải danh mục từ tài khoản (BE)
  Future<void> _loadCategories() async {
    final result = await _taskService.getCategories(1); // Giả định userId = 1
  ...
  Future<void> onSaveTask() async {
    ...
    try {
      ...
      if (taskToEdit == null) {
        result = await _taskService.createTask(1, taskData);
      } else {
      ...
      if (mounted) { setState(() => isLoading = false); _showSnackBar("Lỗi kết nối Server!"); }
    }
  }
  ...
  // Selector chỉ hiển thị danh sách từ tài khoản
